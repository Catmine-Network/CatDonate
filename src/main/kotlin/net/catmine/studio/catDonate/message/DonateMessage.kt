...
enum class DonateMessage(
    ...
    PREVIOUSLY_REJECTED_CARD("previously-rejected-card", "<red>Thẻ này đã bị từ chối trước đó: <white>{reason}</white>. Vui lòng không gửi lại."),
    ...
    FAILED("failed", "<red>Nạp thẻ không thành công: <white>{reason}</white>. Vui lòng kiểm tra lại thông tin thẻ."),
    SUBMISSION_FAILED("submission-failed", "<red>Không thể gửi thẻ lên hệ thống: <white>{reason}</white>. Vui lòng thử lại; nếu vẫn lỗi, hãy liên hệ quản trị viên.</red>"),
    ...
    STATUS_SUBMISSION_FAILED("status-values.submission-failed", "gửi thẻ thất bại"),
    ...
    ADMIN_PLAYER_HISTORY_HEADER("admin-player-history-header", "<gold>Lịch sử của {player}</gold> <gray>({player_id})</gray><newline><yellow>Tổng nạp thành công: <green>{total}</green> • {success_count}/{count} giao dịch thành công</yellow>"),
    ADMIN_PLAYER_HISTORY_ENTRY("admin-player-history-entry", "<gray>#{number} • {created_at}</gray> <gold>{request_id}</gold> — <aqua>{telco}</aqua> <white>{amount}</white> — <yellow>{status}</yellow>"),
    ADMIN_PLAYER_HISTORY_NOT_FOUND("admin-player-history-not-found", "<red>Không tìm thấy lịch sử nạp của người chơi {player}."),
